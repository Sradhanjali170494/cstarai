...
import { GET_ALL_SPONSORS, DELETE_SPONSOR } from './../../../services/queries';
...
export default function Page() {
    ...
    const { loading, error, data, refetch } = useQuery(GET_ALL_SPONSORS, {      
    ...
    const fetchData = async (refetch) => {
        try {
            await refetch();
            if(data){
                const responseData = data.getAllSponsors;
                setSponsors(responseData.sponsors);
                setTotalCount(responseData.totalCount);
            }
        } catch (error) {
            console.log('Error fetching sponsors data:', error.message);
            if(error.message){
                alert(error.message);
            }
        }
    ...
    const editData = (id) => {
       router.push({ pathname: '/home/sponsors/edit', params: { id: id } });
    }
            ...
            <ScrollView>
                <View>
                    ...
                    >
                        ...
                        <View style={{ flex: 1 }}>
                            <Button icon="plus" mode="contained" buttonColor ="#000" style={styles.addBtn} textColor='#fff' onPress={() => router.push("home/sponsors/add")} >
                                Add New Sponsor
                            </Button>
                        </View>
                                ...
                                <DataTable style={{ backgroundColor: '#fff' }}>
                                    ...
                                    </DataTable.Header>
                                    {
                                        sponsors && (sponsors.length > 0) ? (   
                                            sponsors.map((item) => {
                                                return (
                                                    <DataTable.Row key={item.sponsor_id}>
                                                        <DataTable.Cell textStyle={styles.cellItems}>{capitalizeWords(item.contact_first_name + ' ' + item.contact_last_name)}</DataTable.Cell>
                                                        <DataTable.Cell textStyle={styles.cellItems}>{item.contact_email}</DataTable.Cell>
                                                        <DataTable.Cell textStyle={styles.cellItems}>{formatPhoneNumber(item.contact_phone)}</DataTable.Cell>
                                                        ...
                                                        {Platform.OS == 'web' ? (
                                                            <DataTable.Cell>
                                                                <IconButton
                                                                    ...
                                                                    iconColor="#7D7DD5"
                                                                    size={20}
                                                                    onPress={() => editData(item.sponsor_id)}
                                                                />
                                                                <IconButton
                                                                    icon="delete"
                                                                    mode="contained"    
                                                                    iconColor="#DC8A8A"
                                                                    size={20}
                                                                    onPress={() => showDeleteDialog(item.sponsor_id)}
                                                                />
                                                            </DataTable.Cell>
                                                        ) : (
                                                            <>
