...
import { GET_VENUE_DATA, UPDATE_VENUE, UPLOAD_FILE } from './../../../services/queries';
...
export default function Page() {
    ...
    const [venueDet, setVenueDet] = useState(false);

    const { error, data, refetch } = useQuery(GET_VENUE_DATA, {
    ...
    const fetchData = async (refetch) => {
        try {
            await refetch();
            if (data) {          
                const venueDetails = data.getVenue;

                const getVenueLogo = venueDetails?.logo ? venueDetails?.logo : [];
                const getVenueBanner = venueDetails?.banner ? venueDetails?.banner : [];

                const updatedFormValues =
                {
                    venueName: venueDetails?.venue_name,
                    venueCompanyName: venueDetails?.company_name,                 
                    venueContactFirstName:  venueDetails?.contact_first_name,
                    venueContactLastName:  venueDetails?.contact_last_name,
                    venueContactEmail:  venueDetails?.contact_email,
                    venueContactPhone: venueDetails?.contact_phone?formatPhoneNumber(venueDetails?.contact_phone):'',
                    venueStatus: venueDetails?.status,
                    venueAddress1:venueDetails?.address1,
                    venueAddress2: venueDetails?.address2,
                    venueCountry: venueDetails?.country,
                    venueState: venueDetails?.state,
                    venueZip: venueDetails?.zip,
                    venueLogo: getVenueLogo,
                    venueBanner: getVenueBanner,
                }
    ...
    const handleUpdateVenue = async (venueData) => {
        try {
            ...
            if (result.data.updateVenue.venue_id) {    
